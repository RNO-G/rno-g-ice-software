...
/** File-scope (not local to mon_thread()) so that mon_thread() can free its
 *  scaler_v_mem after the acquisition loop exits.
 */
...
static void radiant_flower_servo(double nowf)
{
  ...
  // these statics would be unguarded shared state (a race condition). rad_servo_state
