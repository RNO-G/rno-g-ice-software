...
#define WRITE_ENUM(X,Y,COMMENT, STRS)\
 ...
 SECT(radiant,"RADIANT configuration");
  ...
  SECT(readout,"Readout settings for the RADIANT");
    ...
    SECT(cap_trigger_rate,"Trigger rate cap: skip readout of RF-triggered events once their rate is too high, tracked separately per RF trigger source below. soft/ext/pps triggers are never skipped. Triggers ambiguously attributed to RADIANT0 or RADIANT1 (RFX) are not independently rate-capped.");
      SECT(RF0,"Trigger rate cap for RADIANT's internal RF trigger 0");
 ...
 UNSECT() ;
  ...
  SECT(lt,"Settings for the low-threshold (FLOWER) board");
    ...
    SECT(servo, "Servo settings for the low-threshold board");
       ...
       WRITE_FLT(lt.servo,fast_scaler_weight,"Weight of fast (100Hz) scalers in calculating PID goal");
       WRITE_FLT(lt.servo,slow_scaler_weight,"Weight of slow (1Hz) scalers in calculating PID goal");
